                ...
                <div class="col-sm-8 col-sm-offset-2">                    
                    ...
                    <div class="row m0 flexslider" id="productImageSlider">
                        <ul class="slides">
							...
							<li><img data-tilt src="<?=SITE_URL;?>upload/<?=$product['photo'];?>" alt="<?=$product['name'];?>"></li>
                            <?php
								$product_img_sql=sqlfetch("SELECT * FROM `subproduct` where subcat='$cid' order by fld_order ");
								if(count($product_img_sql))
								foreach($product_img_sql as $product_img){
							?>
							<li><img src="<?=SITE_URL;?>upload/<?=$product_img['photo'];?>" alt="<?=$product['name'];?>"></li>
							<?php
								}
							?>
                        </ul>
                    ...
                    <div class="row m0 flexslider" id="productImageSliderNav">
                        <ul class="slides">
                            <li><img class="img-thumbnail" src="<?=SITE_URL;?>upload/<?=$product['photo'];?>" alt="<?=$product['name'];?>"></li>
							<?php 
								if(count($product_img_sql))
								foreach($product_img_sql as $product_img){
							?>
                            <li><img class="img-thumbnail" src="<?=SITE_URL;?>upload/<?=$product_img['photo'];?>" alt="<?=$product['name'];?>"></li>
                           <?php
								}
							?>
                        </ul>
                        ...
                        <div class="row m0 titleRow">
                            ...
                            <div class="fleft titlePart">
                                <a><h4 class="blogTitle heading"><?=$product['name'];?></h4></a>
